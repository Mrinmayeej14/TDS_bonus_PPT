...
def _llm_instruction(text: str, guidance: str, target_slides: Optional[int]) -> str:
    """Create the LLM instruction string for slide planning."""
    count_line = (
        f"- Choose exactly {target_slides} slides (if content is short, expand; if long, summarize).\n"
        if target_slides
        else f"- Choose a reasonable number of slides (min {MIN_SLIDES}, max {MAX_SLIDES})\n"
    )
    return f"""
...
def _extract_openai_output_text(resp: Any) -> str:
    """Try multiple shapes of OpenAI Responses API objects to extract text."""
...
def _safe_json_parse(s: str) -> Dict[str, Any]:
    """Parse JSON from a string, with a tolerant fallback that extracts the first {...} block."""
    s = (s or "").strip()
    if not s:
        return {"slides": []}
    try:
...
async def build_slide_plan(
    ...
    """Call the chosen provider to generate a slide plan (JSON-only)."""
    model_name = (
        model
        or (
...
async def build_slide_plan_with_retry(
    ...
    """Retry wrapper around build_slide_plan with simple linear backoff."""
...
def _rect(left: int, top: int, width: int, height: int) -> Dict[str, int]:
    """Return a normalized rectangle dict."""
    return {
        "left": max(0, left),
        "top": max(0, top),
        "width": max(0, width),
...
def _overlaps_any_text(img: Dict[str, int], zones: List[Dict[str, int]], thresh: float = 0.10) -> bool:
    """Check if an image rectangle overlaps any text zone beyond a threshold."""
...
async def generate_pptx(
    ...
    """Generate a PPTX by planning slides with an LLM and rendering them with python-pptx."""
    # Validate text
